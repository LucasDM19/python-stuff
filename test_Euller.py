...
class TestProblem9(unittest.TestCase):
   def testGeneratePitagoreanTripleNegative(self):
      self.assertEqual( generatePitagoreanTriple(-1, 1), (-1, -1, -1) ) #self.assertRaises(ValueError, generatePitagoreanTriple, [-1, 1] )
      self.assertEqual( generatePitagoreanTriple(1, -1), (-1, -1, -1) ) #self.assertRaises(ValueError, generatePitagoreanTriple, [1, -1] )
   def testGeneratePitagoreanTripleInvalid(self):
      self.assertEqual( generatePitagoreanTriple(2, 80), (-1, -1, -1) ) #self.assertRaises(ValueError, generatePitagoreanTriple, [2, 80] )
   def testGeneratePitagoreanTripleBasic(self):
      self.assertEqual( generatePitagoreanTriple(2, 1), (3, 4, 5) )
   def testProblem9_60(self):
      self.assertEqual( problem9(12), (3, 4, 5, 60) )
   def testProblem9_1000(self):
      self.assertEqual( problem9(1000, maxTry=500), (375, 200, 425, 31875000) )
   
class TestProblem10(unittest.TestCase): 
   def testgetPrimesBelow10(self):
      self.assertEqual( getPrimesBelow(10), [2, 3, 5, 7] )
   def testProblem10_10(self):
      self.assertEqual( problem10(10), 17 )
   #def testProblem10_2000000(self):
      #self.assertEqual( getPrimesBelow(2000000), -1 )
   def testProblem10_2000000(self):
