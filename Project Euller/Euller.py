...
def obtemVizinhos(matriz, i, j):
   max_i = len(matriz)
   max_j = len(matriz[0])
   vizinhos = []
   if((i+3 <= max_i)): vizinhos.append( [matriz[i+0][j], matriz[i+1][j], matriz[i+2][j], matriz[i+3][j]] ) # Abaixo
   if((j+3 <= max_j)): vizinhos.append( [matriz[i][j+0], matriz[i][j+1], matriz[i][j+2], matriz[i][j+3]] ) # Direita
   if((i+3 <= max_i) and (j+3 <= max_j)): vizinhos.append( [matriz[i+0][j+0], matriz[i+1][j+1], matriz[i+2][j+2], matriz[i+3][j+3]] ) # Diagonal
   if((i >= 3) and (j+3 <= max_j)): vizinhos.append( [matriz[i-0][j+0], matriz[i-1][j+1], matriz[i-2][j+2], matriz[i-3][j+3]] ) # Diagonal2
   return vizinhos

def problem11():
   max_produ = 0
   matriz=convertStringToMatrix("""08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00
81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65
52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91
22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48""")
   for i in range(len(matriz)):
      for j in range(len(matriz[0])):
         vizinhos = obtemVizinhos(matriz, i, j)
         for viz in vizinhos:
           produt = 1 
           for v in viz:
            produt *= v
